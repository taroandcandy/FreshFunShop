<template>
    ...
    <scroll-view scroll-y class="viewport">
        <!-- 收货地址 -->
        <navigator v-if="selectedAddress" url="/pagesMember/address/index?from=order" hover-class="none"
            class="shipment">
        ...
        <view class="goods">
            <!-- 点击商品跳转到商品详情 -->
            <navigator v-for="item in orderData?.goods" :key="item.skuId"
                :url="`/pages/goodsDetail/index?id=${item.id}`" class="item" hover-class="none">
        ...
        <view class="related">
            ...
            <view class="item">
                ...
                <input class="input" :cursor-spacing="30" placeholder="选填，建议留言前先与商家沟通确认" v-model="buyerMessage">
            </view>
        </view>
        <!-- 支付金额 -->
        <view class="settlement">
            <view class="item">
                ...
                <!-- .toFixed(2)：把数字格式化为两位小数的字符串（例如 12.3 → "12.30"） -->
...
const getMemberOrderPreData = async () => {
    ...
    if (query.skuId && query.count) {
        ...
        uni.navigateTo({ url: `/pagesOrder/detail/index?id=${query.orderId}` })
    } else {
        // 用于购物车
...
const selectedAddress = computed(() => {
    return addressStore.selectedAddress || orderData.value?.userAddresses.find((item) => item.isDefault)
})
// 提交订单回调
