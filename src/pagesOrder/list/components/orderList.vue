    ...
    <scroll-view scroll-y class="orders" refresher-enabled @refresherrefresh="onRefresherrefresh"
        ...
        <view class="card" v-for="item in orderList" :key="item.id">
            ...
            <view class="status">
                ...
                <!-- 状态:待评价/已完成/已取消  可以展示删除订单 -->
                <text class="icon-delete" v-if="item.orderState >= OrderState.DaiPingJia"
                    @tap="onDeleteOrder(item.id)"></text>
            </view>
            <!-- 商品信息，点击商品跳转到订单详情，不是商品详情, 所以用外层循环item的id -->
            <navigator v-for="sku in item.skus" :key="sku.id" class="goods"
                :url="`/pagesOrder/detail/index?id=${item.id}`" hover-class="none">
                <view class="cover">
            ...
            <view class="action">
                ...
                <template v-else>
                    ...
                    <view class="button delete" v-if="item.orderState >= OrderState.DaiPingJia"
...
const getMemberOrderListData = async () => {
    ...
    try {
        // 记录本次要请求的页码，避免并发时用错
        const pageToFetch = queryParams.page!
        const res = await getMemberOrderListAPI({ ...queryParams, page: pageToFetch })

        // 防止过期响应覆盖，污染数据（可选）
        if (pageToFetch !== queryParams.page) return
        // 数据追加
